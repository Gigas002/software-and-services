...
This is a list of software, services and some minor tweaks which I use. I made it in for myself to have a backup for most important thing and have a quick startup if something breaks.

List mostly targets arch-like linux distributives, but there are also some windows apps. Most part of list if FLOSS and I treat this as default choice, but there are some proprietary apps just in case/no alternative, I'll mark theme correspondingly.

A lot of stuff and recommendations on privacy was taken from [here](https://www.privacyguides.org/). Also a lot of useful software and tweaks for arch-based distributives listed on [arch wiki page](https://wiki.archlinux.org/title/List_of_applications).
...
Read [this article](https://www.privacyguides.org/operating-systems/#firmware). I don't use it because my router isn't supported by these systems yet.
...
- [Mozilla Firefox](https://www.mozilla.org/en-US/firefox/new/) -- preinstalled on most linux distibutives
...
- [Onion browser](https://github.com/onionbrowser/onionbrowser) -- Tor browser for iOS
...
- [uBlock](https://github.com/gorhill/uBlock) -- ad blocker
...
- $FREEMIUM$ [Firefox relay](https://github.com/mozilla/fx-private-relay) -- not tested this one yet, but looks interesting
...
I'm using user.js from this repo in my firefox's everyday life. If you want to see, on which it was based see [arkenfox's user.js](https://github.com/arkenfox/user.js). Don't miss the wiki there, there are plenty of good advices.
Plus, I recommend enable DNS over HTTPS in browser/device/router connection settings.
...
First, select your DNS provider from the link above. I personally prefer [AdGuard DNS](https://github.com/AdguardTeam/AdGuardDNS/) over Cloudflare since these are open source, support DoH, DoT and DNSCrypt and fast, though keep some logs for 24 hours. As an alternative I use [BlahDNS](https://github.com/ookangzheng/blahdns/) which is said to "not collect any logs", but it's speed is slower, so it's on the second place in priority. Plus, you can enable **DNS over HTTPS** in firefox settings (which is redundant if you've set up it on router level).
...
- !LINUX ONLY! [ClamTK](https://github.com/dave-theunsub/clamtk) -- GUI for ClamAV, works only on Linux
- !LINUX ONLY! [gufw](https://github.com/costales/gufw) -- firewall for linux systems, which is a GUI for [ufw](https://wiki.archlinux.org/title/Uncomplicated_Firewall)
- !PROPRIETARY! !WINDOWS ONLY! Windows defender and firewall (WIN10+) -- handles most cases pretty good without any need of additional software
...
- $FREEMIUM$ [ProtonMail](https://github.com/ProtonMail) -- can't say I recommend it's free plan, since it doesn't allow you to do lots of things, but paid plans are awesome. *Plus* plan lets you use [new ProtonDrive service](https://protonmail.com/blog/proton-drive-early-access/), more info about it in *File sync* section below
- [Disroot](https://disroot.org/en/services/email) -- nice free alternative for ProtonMail
...
Read more [here](https://www.privacyguides.org/software/file-sharing/).
...
Read more [here](https://www.privacyguides.org/providers/search-engines/).

- [DuckDuckGo](https://github.com/duckduckgo) -- default choice on Tor Browser, supports onion services. Partly proprietary
...
If you need an alternative for services like twitter/facebook/etc, check out [this article](https://www.privacyguides.org/providers/social-networks/). Here I'll mainly cover some alternative frontends for these mainstream services, not introducing completely new and already good ones.
...
AntiDupl.NET, gimp/krita, KDE's video editor, K-Lite Codec Pack, vlc, ImageGlass
...
- [yt-dlp](https://github.com/yt-dlp/yt-dlp) -- fork of abandoned [youtube-dl](https://github.com/blackjack4494/yt-dlc) for downloading videos; CLI
...
While bash/zsh on linux works good on it's own, there are alternatives for windows. As KDE user I prefer *konsole* + *zsh*. Only advice to arch users is to uncomment `Color` string in you `/etc/pacman.conf`. Seriously!
...
- !WINDOWS ONLY! [Windows Terminal](https://github.com/Microsoft/Terminal) -- terminal for windows system. Supports WSL, git bash, etc and is actually good.
